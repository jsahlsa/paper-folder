...
const calculateThickness = (e) => {
    ...
    console.log(feet, 'hi');
    if (miles >= 92955807.267433) {
        thickness.textContent = `${au} AUs ${remainderEarths} earths`;
    } else if (miles >= 7917.5) {
        thickness.textContent = `${earths} earths ${remainderMiles} miles`;
    } else if (feet >= 5280) {
        thickness.textContent = `${miles} miles ${remainderFeet} feet`;
    } else if (inches >= 12) {
        thickness.textContent = `${feet} feet ${remainderInches} inches`;
    } else {
        thickness.textContent = `${inches} inches`;
    }
};
